...
import cn.bmob.ad.R;
import cn.bmob.ad.util.APIUtil;


public abstract class FeedsAbsListView extends FeedsAdapterView<ListAdapter>
	...
	private FeedsListView.OnScrollListener mOnScrollListener;
	...
	private int mLastScrollState = FeedsListView.OnScrollListener.SCROLL_STATE_IDLE;
	...
	private FeedsListView.OnStartListener onRefreshStart;
	private FeedsListView.OnStartListener onLoadMoreStart;
	private FeedsListView.OnScrollStateListener onScrollStateListener;
	private int mScrollState = FeedsListView.OnScrollStateListener.EDGE;
	...
	public FeedsAbsListView(Context context) {
		super(context);
		initAbsListView();
	}

	public FeedsAbsListView(Context context, AttributeSet attrs) {
		this(context, attrs, android.R.attr.absListViewStyle);
	}

	public FeedsAbsListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initAbsListView();
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.FeedsAbsListView, defStyle, 0);
		Drawable d = a
				.getDrawable(R.styleable.FeedsAbsListView_android_listSelector);
		if (d != null) {
			setSelector(d);
		}
		mDrawSelectorOnTop = a.getBoolean(
				R.styleable.FeedsAbsListView_android_drawSelectorOnTop, false);
		boolean scrollingCacheEnabled = a.getBoolean(
				R.styleable.FeedsAbsListView_android_scrollingCache, true);
		setScrollingCacheEnabled(scrollingCacheEnabled);
		int transcriptMode = a.getInt(
				R.styleable.FeedsAbsListView_android_transcriptMode,
				TRANSCRIPT_MODE_DISABLED);
		setTranscriptMode(transcriptMode);
		int color = a.getColor(
				R.styleable.FeedsAbsListView_android_cacheColorHint, 0);
		setCacheColorHint(color);
		boolean smoothScrollbar = a.getBoolean(
				R.styleable.FeedsAbsListView_android_smoothScrollbar, true);
	...
	public void setOnScrollListener(FeedsListView.OnScrollListener l) {
		mOnScrollListener = l;
	...
	boolean performLongPress(final View child, final int longPressPosition,
		...
		if (mOnItemLongClickListener != null) {
			handled = mOnItemLongClickListener.onItemLongClick(
					(FeedsListView) this, child, longPressPosition, longPressId);
		}
		if (handled) {
	...
	private boolean startScrollIfNeeded(int x, int y) {
		...
		if (distanceX > mTouchSlop || distanceY > mTouchSlop) {
			if (distanceY > distanceX * 2) {
				...
				mSelectorPosition = INVALID_POSITION;
				reportScrollStateChange(FeedsListView.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
		...
		case TOUCH_MODE_SCROLL:
			...
			if (!mFlingRunnable.scrollToAdjustViewsUpOrDown()) {
				...
				if (Math.abs(initialVelocity) > mMinimumVelocity) {
					...
					reportScrollStateChange(FeedsListView.OnScrollListener.SCROLL_STATE_FLING);
					mFlingRunnable.start(-initialVelocity);
				} else {
					mTouchMode = TOUCH_MODE_REST;
					reportScrollStateChange(FeedsListView.OnScrollListener.SCROLL_STATE_IDLE);
					if (mFlingRunnable != null) {
	...
	boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
		...
		if (isOutOfTop || isOutOfBottom) {
			...
			if (onScrollStateListener != null) {
				if (mScrollState != FeedsListView.OnScrollStateListener.EDGE) {
					mScrollState = FeedsListView.OnScrollStateListener.EDGE;
					onScrollStateListener.onChange(FeedsListView.OnScrollStateListener.EDGE);
				}
			}
		} else {
			if (zrcHeader != null) {
			...
			if (incrementalDeltaY > 5) {
				if (onScrollStateListener != null) {
					if (mScrollState != FeedsListView.OnScrollStateListener.UP) {
						mScrollState = FeedsListView.OnScrollStateListener.UP;
						onScrollStateListener
								.onChange(FeedsListView.OnScrollStateListener.UP);
					}
				}
			} else if (incrementalDeltaY < -5) {
				if (onScrollStateListener != null) {
					if (mScrollState != FeedsListView.OnScrollStateListener.DOWN) {
						mScrollState = FeedsListView.OnScrollStateListener.DOWN;
						onScrollStateListener
								.onChange(FeedsListView.OnScrollStateListener.DOWN);
					}
				}
			}
		}
	...
	public void setOnRefreshStartListener(FeedsListView.OnStartListener onStart) {
		this.onRefreshStart = onStart;
	}

	public void setOnLoadMoreStartListener(FeedsListView.OnStartListener onStart) {
		this.onLoadMoreStart = onStart;
	}

	public void setOnScrollStateListener(FeedsListView.OnScrollStateListener onScrollListener) {
	...
	private class FlingRunnable implements Runnable {
		...
		void start(int initialVelocity) {
			...
			mTouchMode = TOUCH_MODE_FLING;
			ViewCompat.postOnAnimation(FeedsAbsListView.this, this);
		}
	...
	class AdapterDataSetObserver extends
			FeedsAdapterView<ListAdapter>.AdapterDataSetObserver {
		@Override
